...
def main(args):
    ...
    if with_OOD:
        ...
        (UCE_OOD, _, 
        UCE_df_OOD,_,
        _,_,_) = get_UCE(OOD_probs_df, OOD_true_classes,len(classifier.unique_classes))
    else:
        UCE_OOD = None
        UCE_df_OOD = None
    ...
    fig_vars = visualize_metric_dist_per_class(dict_vars.copy(),'variances')
    ...
    for class_ in dict_vars:
        # sample one image with small (top 10%) and large (top 5%) var for each class
        idx_class = list(dict_vars[class_].keys())
        idx_num_small = np.random.randint(0,int(len(idx_class)/20),1)
        idx_num_large = np.random.randint(0,int(len(idx_class)/20),1)
    ...
    for i in range(len(idx_true)):   
        ...
        #_, img_true_var = classifier._get_embedding_(objects[i_true],bboxs[i_true])  
        #_, img_false_var = classifier._get_embedding_(objects[i_false],bboxs[i_false])  
    ...
    with open(f'reports/test_results_tune/{file_name}_metrics_dict.pickle', 'wb') as f:
        pickle.dump(metrics_dict, f)
    with open(f'reports/test_results_tune/{file_name}_graphs_dict.pickle', 'wb') as f:
    ...
    wandb.log({'fig_tsne_test': wandb.Image(fig_tsne_test),
               ...
               'Uncertainty plots': wandb.Image(uncert_plots),
    ...
    for class_ in dict_vars:
        wandb.log({'Small variance examples': wandb.Image(img_small_var[class_]),
                   'Large variance examples': wandb.Image(img_large_var[class_]),
                   'Low uncertainty examples': wandb.Image(img_small_uncert[class_]),
