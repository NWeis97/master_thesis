...
class TuplesDataset_2class(data.Dataset):
    ...
    def __init__(self, mode: str, nnum: int=10, qsize_class:int = 20,
    ...
    def create_epoch_tuples(self, net: ImageClassifierNet_BayesianTripletLoss, num_classes_per_neg,
        ...
        for indx,i in enumerate(self.qidxs):
            ...
            if (indx+1) % self.print_freq == 0 or (indx+1) == len(self.qidxs):
                logger.info('>>>> {}/{} done... '.format(indx+1, len(self.qidxs)))
        ...
        for q in range(len(self.qidxs)):
            ...
            while len(nidxs) < self.nnum:
                potential = ranks[r, q]
                class_potential = self.pool_classes_list[self.qidxs[potential]]
                if num_class[class_potential] < num_classes_per_neg:
                    if (torch.rand(1).item() > skip_closeset_neg_prob):
                        nidxs.append(self.qidxs[potential])
